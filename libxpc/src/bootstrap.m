...
#include <pthread.h>
#include <stdlib.h>
// FIXME: file not reimplimented... still uses old launchd apis
xpc_object_t
bootstrap_create_request(mach_port_t bootstrap_port){
...
int
xpc_bootstrap_routine(unsigned int routine, xpc_object_t msg, xpc_object_t* out)
{
  return xpc_interface_routine(5, routine, msg, out, 0, 0);
}
...
{
    xpc_object_t dictionary; // r14
    kern_return_t ret; // er12
    xpc_object_t v11 = NULL; // [rsp+0h] [rbp-30h]
    
    dictionary = bootstrap_create_request(bp);
    ret = KERN_INVALID_RIGHT;
    // set mach port send rights
    if (!xpc_mach_port_make_send(requestor_port)){
        // if kern succeeded, create mach_send object and insert it into dict
        xpc_dictionary_set_mach_send(dictionary, "reqport", requestor_port);
        // ...
        ret = xpc_bootstrap_routine(0xC9, dictionary, &v11);
        if (v11){
            *subset_port = _xpc_dictionary_extract_mach_send(v11, "subsetport");
            xpc_release(v11);
        }
        ret = mach_port_deallocate(mach_task_self(), requestor_port);
//      if ( v9 )
//        _os_assumes_log(v9, "subsetport");
